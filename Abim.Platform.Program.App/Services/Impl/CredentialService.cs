...
namespace Abim.Platform.Program.App.Services.Impl
{
    ...
    public class CredentialService : ServiceBase<Credential, ICredentialRepository>, ICredentialService
    {
        ...
        private bool DeactivateIMCredential(Credential imCred, string createdBy)
        {
            Log.Trace("Entering DeactivateIMCredential.");
            // If IM is already deselected, then do not do anything
            // else De-activate the newest issuance. PBI 201405
            if (imCred.IsDeselected)
            {
                Log.Trace("IMCredential is already Deactivated.");
                Log.Trace("Leaving DeactivateIMCredential.");
                return false;
            }

            var credDeactivationDate = DateTime.Now;
            imCred.NewestIssuance.DeselectionEffectiveDate = credDeactivationDate;
            imCred.NewestIssuance.DeselectionSubmittedDate = credDeactivationDate;

            Log.Trace("Deselecting IMCredential.");
            imCred.Deselect(createdBy, credDeactivationDate, credDeactivationDate);
            Log.Trace("IMCredential Deselection done.");
            Log.Trace("Leaving DeactivateIMCredential");
            return true;
        }
