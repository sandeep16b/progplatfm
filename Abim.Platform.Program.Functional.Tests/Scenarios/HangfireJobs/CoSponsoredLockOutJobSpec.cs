...
namespace Abim.Platform.Program.Tests.Scenarios.HangfireJobs
{
    ...
    [Story(
      ...
      IWant = "to be able to invoke the CoSponsored LockOut Job",
      SoThat = "it can run the CoSponsored Lock Out process"
      )]
    [TestFixture]
    public class CoSponsoredLockOutJobSpec
    {
        [Test]
        public void Should_Query_Database_For_Credential_IDs()
        {
            new ShouldQueryDatabaseForCredentialIDs().BDDfy();
        }
        ...
        private abstract class CoSponsoredLockOutJobSpecBase
        {
            protected CoSponsoredLockOutJob _sut;
            ...
            {
                SetupMocks();
                _sut = new CoSponsoredLockOutJob(
        ...
        private class ShouldQueryDatabaseForCredentialIDs : CoSponsoredLockOutJobSpecBase
        {
        ...
        private class ShouldThrowExceptionWhenCancellationTokenSaysCancel : CoSponsoredLockOutJobSpecBase
        {
        ...
        private class ShouldEnqueueChildJobForEachCredentialId : CoSponsoredLockOutJobSpecBase
        {
            private void WhenTheMethodExecutesNoExceptionShouldHaveBeenCaught()
            {
                _caughtException.Should().BeNull();
