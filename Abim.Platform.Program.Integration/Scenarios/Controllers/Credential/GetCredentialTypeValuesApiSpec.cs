...
namespace Abim.Platform.Program.Integration.Scenarios.Controllers.Credential
{
    [Story(
        ...
        SoThat = "to get a list of the available values for CredentialTypes."
        )]
    [TestFixture]
    public class GetCredentialTypeValuesApiSpec
    {
        [TestCase]
        [WorkItem(78850)]
        public void GetCredentialTypeValuesReturnsOk()
        {
            new GetCredentialTypeValuesReturnsOk().BDDfy();
        }

        [TestCase]
        [WorkItem(78850)]
        public void GetCredentialTypeValuesReturnsForbidden()
        {
            new GetCredentialTypeValuesReturnsForbidden().BDDfy();
        }

        [TestCase]
        [WorkItem(78850)]
        public void GetCredentialTypeValuesAnonymousReturnsUnauthorized()
        {
            new GetCredentialTypeValuesAnonymousReturnsUnauthorized().BDDfy();
        }
    }
    ...
    public abstract class GetCredentialTypesScenario : CredentialControllerScenario
    {
    }
    ...
    /// </summary>
    public class GetCredentialTypeValuesReturnsOk : 
        GetCredentialTypesScenario
    {
        IEnumService EnumService { get; set; }
        EnumTypeResponseResource<CredentialType> Resource { get; set; }
        ...
        {
            Url = "/api/v1.0/Credential/enum/CredentialTypes";
        }

        public async Task WhenICallGetAllCredentialTypeValues()
        {
            Result = await HttpServer.CreateRequest(Url)
                .AddHeader("Authorization", "Bearer " + Token)
                .GetAsync();
            ResponseContent = await Result.Content.ReadAsStringAsync();
            Resource = JsonConvert.DeserializeObject<EnumTypeResponseResource<CredentialType>>(ResponseContent);
        }
        ...
        {
            foreach(var enumValue in Enum.GetValues(typeof(CredentialType)))
            {
    ...
    public class GetCredentialTypeValuesReturnsForbidden :
       GetCredentialTypesScenario
    {
        IEnumService EnumService { get; set; }
        EnumTypeResponseResource<CredentialType> Resource { get; set; }
    ...
    public class GetCredentialTypeValuesAnonymousReturnsUnauthorized
        : GetCredentialTypesScenario
    {
        IEnumService EnumService { get; set; }
        EnumTypeResponseResource<CredentialType> Resource { get; set; }
