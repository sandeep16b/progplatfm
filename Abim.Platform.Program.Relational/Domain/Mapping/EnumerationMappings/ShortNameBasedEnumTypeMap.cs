...
{
    /// <summary>
    /// The use of this mapping class indicates that the enum's DisplayAttribute.ShortName corresponds to the database value
    /// </summary>
    /// <typeparam name="T"></typeparam>
    /// <seealso cref="Abim.Platform.Program.Relational.Domain.Mapping.EnumerationMappings.EnumTypeMap{T}" />
    public class ShortNameBasedEnumTypeMap<T> : EnumTypeMap<T>
        where T : struct, IConvertible, IComparable, IFormattable
    {
        ...
        {
            return EnumAttributes.GetEnumByShortName<T>(code.ToString());
        }
        ...
        {
            return EnumAttributes.ReadEnumShortName(code);
        }
    }
}
